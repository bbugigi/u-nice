...
const App = (() => {
  ...
  const Products = (() => {
    const getEmoji = (name) => {
      const map = { "Wins Town Women's Probiotic Gummies (60 Gummies)":'💊', "Wins Town Collagen with Burn":'🔥', "5D Gluta Collagen Glowing Powder (1 kg)":'✨', "Duozi Marine Collagen+ 5-in-1":'🐟', "Multi Collagen Peptides Powder (1 lb / 454g)":'💪', "Gluta Master Collagen Glowing Powder - Vanilla (1 kg)":'🌟', "5D Gluta Plant-Based Protein 100% Whey (1 kg)":'🥤', "Daynee Evening Primrose Oil Gummies (60s)":'🌙', "Daynee Calcium + D3 Gummies":'🦴', "Daynee Magnesium Glycinate Gummies (60s, 600mg)":'😴', "Daynee Vitamin C Gummies (60s, 250mg)":'🍊', "Jiake Ultimate Maca Protein Powder (300g)":'🍑', "WinsTown / Daynee Assorted Health Gummies (Display Set)":'🍬', "MTM-100 Sildenafil Citrate Tablets (100mg, 4s)":'💊', "CeraVe Moisturising Cream (16 oz / 454 g)":'🧴', "Osckal Alpha Arbutin 3 Plus Whitening Body Soap":'🧼', "Sadoer Tea Tree Acnes Oil Control Face Cream":'🌿', "Dr. Meinaier Vitamin C & Pure Collagen Sugar Scrub (650g)":'🧖', "Dove Exfoliating Body Polish / Body Scrub":'💐', "Kormesic 100% Raw Batana Oil":'🫒', "Best Collagen Serum Body Oil (Yellow)":'💛', "Best Collagen White Body Oil Extra Whitening (Pink, 500ml)":'🩷', "Sajed Mamool Incense (100 g)":'🪔', "WinsTown Slim Plus Fat Burner (60 Capsules)":'🔥' };
      return map[name] || '📦';
    };
    ...
    const renderCarousels = async () => {
      ...
      const cats = { supplements:'supplementsCarousel', skincare:'skincareCarousel', bodycare:'bodycareCarousel', wellness:'wellnessCarousel' };
    ...
    const openDetail = async (id) => {
      ...
      document.getElementById('productDetailBody').innerHTML = `
        <div class="relative"><img src="${p.image_url}" alt="${name}" class="w-full h-56 object-cover">
  ...
  const Search = (() => {
    ...
    const showAutocomplete = (query) => {
      ...
      let html = '';
      if (matches.length) {
        html = matches.map(p => `<div class="px-4 py-2 text-sm hover:bg-gray-50 cursor-pointer flex items-center gap-3" onclick="App.Products.openDetail(${p.id});document.getElementById('searchAutocomplete').classList.add('hidden')"><img src="${p.image_url}" class="w-8 h-8 rounded object-cover" alt=""><div><p class="font-medium text-heading">${Products.getName(p)}</p><p class="text-xs text-gray-400">${CURRENCY} ${p.price}</p></div></div>`).join('');
      }
  ...
  const init = async () => {
    ...
    if (typeof Swiper !== 'undefined') {
      try {
        ...
        new Swiper('.supplementsSwiper', { ...sc, navigation: { nextEl: '.supplements-next', prevEl: '.supplements-prev' } });
